...
# 总帐号 负责分发 bnb 
Bnb_TokenGas='9c94be624fc0b0527dfd0db70c0b70d4180e290e30e9c13f688a01fe84e4a5be'
...
AmountBuy={'defalut':60,'start': 1,'end':1} #每次购买量
# AmountBuy1=60
...
class DefiAuto(object):
    ...
    def __init__(self):
        ...
        self.TokenNumber = 0
        self.TokenNumberTotal=0 #1 从0开始算数 一共多少 参与买卖账号
    ...
    def  Firfox(self):
        ...
        self.driver = webdriver.Firefox(executable_path='/root/djangotokens/apiV1/DeFi2/geckodriver', firefox_options=self.options,proxy= self.proxiesFirfox)
    ...
    def buyTokensBuildTransaction(self):  
        try:         
            ...
            # self.account
               # 查看bnb 数量   56808836480000000     0.05
            token_balance = self.web3.eth.get_balance(self.walletAddress)
            if token_balance < 0.3 :
                self.transfer_bnbGas(0.3)
                
            
            # tokenToSell=self.tokenToBuy # 购买数量
            tokenToSell=16808836480000000 # 购买数量
    ...
    def buyTokensSend_raw_transaction(self):  
        try:
            ...
            result = [time1,self.web3.to_hex(tx_token), f"buyTokens {self.web3.fromWei(tokenToSell, self.TradingTokenDecimal)} {self.symbol}",self.walletAddress]
            print(result) 
            return 'ok'
        except Exception as e:  
            self.changeIpweb3() 
