...
sellPrice={'price':1.90,'up':1,'down':0.2,'upStart':1,'upEnd':10}
...
class DefiAuto(object):
    ...
    def buyTokens(self):
        ...
        toBuyBNBAmount=100000000000000
        pancakeSwap_txn = self.contractPancake.functions.swapExactETHForTokens(toBuyBNBAmount,
                                                                        [self.WBNB_Address, self.TokenToSellAddress],
                                                                        self.walletAddress,
                                                                        (int(time.time() + 10000))).build_transaction({
            'from': self.walletAddress,
