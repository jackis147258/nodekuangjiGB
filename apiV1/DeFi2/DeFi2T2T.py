...
buyPrice={'price':1.7,'up':1,'down':0.2,'upStart':1,'upEnd':10}
sellPrice={'price':1.7,'up':1,'down':0.2,'upStart':1,'upEnd':10}
coinsRecharge=0.03 # 子币 每次充值
AmountBuy={'defalut':60,'start': 0.002,'end':0.003} #每次购买量
# AmountBuy1=60
AmountSell={'defalut':60,'start':1,'end':2} #每次出售量
...
class DefiAuto(object):
    ...
    # def nowAccount(self):
        ...
        time1 = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        result=[time1,'当前交易地址:',self.walletAddress,'余额bnb:',token_balance ]
        print(result)        
        ...
        self.buyTokensBuildTransactionswapExactTokensForTokens()
    ...
    def buyTokensBuildTransactionswapExactTokensForTokens(self):  
        try:         
            ...
            pancakeSwap_txn = self.contractPancake.functions.swapExactTokensForTokens(
                ...
                [self.USDT_Address, self.TokenToSellAddress],
                self.walletAddress,
    ...
    def sellTokensBuildTransaction(self):  
        try:     
            ...
            pancakeSwap_txn = self.contractPancake.functions.swapExactTokensForTokens(
                tokenToSell, 0,
                [self.TokenToSellAddress, self.USDT_Address],
    ...
    def sellTokensSend_raw_transaction(self):  
        try:  
            ...
            try:
                ...
                result = [self.web3.to_hex(tx_token), f"Sold {self.web3.fromWei(tokenToSell, self.TradingTokenDecimal)} {self.symbol}"]
                print(result)
                return result
            ...
            # tx_token = self.web3.eth.send_raw_transaction(self.signed_txnSell.rawTransaction)
            # time1 = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            # # result = [time1,self.web3.to_hex(tx_token), f"Sold {self.web3.fromWei(tokenToSell, self.TradingTokenDecimal)} {self.symbol}",self.walletAddress]
            
            # result=[time1,'售出成功',f"Sold {self.web3.fromWei(tokenToSell, self.TradingTokenDecimal)} {self.symbol}",'售出地址:',self.walletAddress,'hash:',self.web3.to_hex(tx_token)]
