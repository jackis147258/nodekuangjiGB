...
from .models import T_task,T_trade,T_Quantify1,T_TokenAddr
...
def convert_integer_str_to_18_decimal_places(integer_str):
    # Ensure the input is string
    integer_str = str(integer_str)

    # Insert the decimal point at the 18th position from the right
    if len(integer_str) > 18:
        result = integer_str[:-18] + "." + integer_str[-18:]
    else:
        # If the input length is less than 18, prepend zeros
        result = "0." + integer_str.rjust(18, '0')
...
class T_TokenAddrAdmin(AjaxAdmin):
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name == "category":
            # 例如，假设Author模型有一个boolean字段`is_active`
            kwargs["queryset"] = CategoryToken.objects.filter(uid=request.user)
    ...
    actions = ('import_data','layerCreateToken','layerImputationToken','layerImputationTokenYmii','allTokenPrice','TokenOtherPrice',TokenBuySellZero,export_excel)
    ...
    def layerImputationToken(self, request, queryset):        
        ...
        ImputationList=GetBnbWeb.GetBnb(request,ImputationToken,token_private_list)   
...
# 归集ymii  或其他代币
    ...
    def layerImputationTokenYmii(self, request, queryset):        
        ...
        ImputationList=GetBnbWeb.GetYmii(request,ImputationToken,tokenAddress,token_private_list)   
        reMsg=reMsg+str(len(ImputationList)) 
    ...
    def import_data(self, request, queryset):      
        ...
        data_lines = [element.strip() for element in data_text.split(',')]
        ...
        with transaction.atomic():
            for line in data_lines:
                ...
                TOKEN_private,TOKEN_ADDRESS =createWallet.dbWallet(request,token_private_data)
                
                # 判断是否存在 
                # 检查是否存在名称为'abd'的记录
                existing_record = T_TokenAddr.objects.filter(TOKEN_ADDRESS=TOKEN_ADDRESS).first()
    ...
    TokenOtherPrice.short_description = '代币余额'
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        # 不为空 在设置 默认值
        if  categoryList:            
            self.TokenOtherPrice.layer['params'][0]['value'] = '0x768a62a22b187EB350637e720ebC552D905c0331'
