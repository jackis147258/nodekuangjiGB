...
def userBalance():
    ...
    try:
        ...
        owner_private_key = config('TOKEN_privateOwner', default='')
        # 初始化 Web3 连接
        ...
        # 从数据库中获取地址和余额集合，假设为一个列表，每个元素为 (address, amount) 的元组
        ...
        # 定义调用合约函数的参数
        addrs = [web3.to_checksum_address(addr) for addr, _ in addresses_and_amounts]
        amounts = [amount for _, amount in addresses_and_amounts]

        # method_to_call = contractPancake.functions.ownerSetUserStakeAB0(username)  # 传入 username 参数
        method_to_call = contractPancake.functions.setBalances([addrs, amounts])  # 传入 username 参数
